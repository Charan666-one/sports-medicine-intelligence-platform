...
**Post-B1/B4 follow-up verification** (after the storage-driver and metrics-endpoint work below): `npm run typecheck`/`lint`/`test`/`build` re-run clean — 66/66 tests (60 original + 4 storage + 2 metrics), 0 type/lint errors, production build unchanged. `GET /api/metrics` smoke-tested against the live dev server (not just unit tests) and confirmed to expose correctly-labeled `http_request_duration_seconds` and `ingestion_jobs_total` series.
