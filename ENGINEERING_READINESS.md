...
| Data integrity | 4/5 | Idempotent ingestion, checksum dedup, dead-letter handling, field-level AES-256-GCM encryption, provenance-tagged AI predictions. File storage is local disk (not durable — see Blocker B1). |
| Test coverage | 2/5 | 60 tests passing, 0 lint/type errors, but only **24.2%** statement coverage overall (measured this session). Controllers/routes are almost entirely untested; coverage is concentrated in pure statistical/crypto/validation modules. |
| Operability | 2/5 | Liveness + readiness health checks, request-ID correlation, structured logs exist. No metrics endpoint, no dashboards, no alerting, no load testing. |
...
- **B1 — File storage is local disk, not durable.** `upload.service.ts` writes uploaded medical reports to `uploads/medical_reports/` on the container's own filesystem via `multer.diskStorage`. On most PaaS targets (Render included, per this repo's own `render.yaml`/`DEPLOY.md`) this is ephemeral and is **not shared** between the API and worker processes if they ever scale to multiple instances — a real risk of silent data loss or a worker unable to find a file the API just wrote. This needs object storage (S3-compatible) before production use. **Not implemented this session** — scoped in the original directive (Phase 2) but not reached; flagging honestly rather than claiming otherwise.
...
- **B4 — No operational metrics/alerting beyond health checks.** There is no `/metrics` endpoint, no dashboard, and no alerting on error rate, latency, or queue depth. An operator would currently find out about a production incident from a user report, not from monitoring. **Not implemented this session.**
- **B5 — Test coverage gap (§5) is a blocker, not just a note, for a system of this sensitivity.** 24.2% statement coverage, concentrated away from controllers, means a real class of regressions (auth/permission logic in route handlers, response shape drift) would not be caught by CI today. The authorization integration suite mitigates but does not close this.
